...
function fetchPostById(id: string): Promise<{ data: { success: boolean; post: IPost } }> {
  return axios.get(`/api/posts/user_posts/${id}`);
}

function fetchPostByUserId(id: string): Promise<{ data: { success: boolean; posts: IPost[] | [] } }> {
  return axios.get(`/api/posts/by_user?user_id=${id}`);
}
